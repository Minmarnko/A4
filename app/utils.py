...
def calculate_similarity(model, tokenizer, sentence_a, sentence_b, device):
    ...
    u, _, _ = model(inputs_ids_a, segment_ids[:1], masked_pos[:1])
    v, _, _ = model(inputs_ids_b, segment_ids[:1], masked_pos[:1])

    # Get the mean-pooled vectors
    u = mean_pool(u, attention_a).detach().cpu().numpy().flatten()
    v = mean_pool(v, attention_b).detach().cpu().numpy().flatten()
...
def predict_nli(model, tokenizer, premise, hypothesis, device):
    """Predicts the relationship between a premise and a hypothesis using a BERT-based model."""
    model.eval()

    # Tokenize sentences
    premise_encoded = tokenizer.encode([premise])
    hypothesis_encoded = tokenizer.encode([hypothesis])

    # Convert to tensors and move to device
    premise_tokens = torch.tensor(premise_encoded["input_ids"][0]).unsqueeze(0).to(device)
    hypothesis_tokens = torch.tensor(hypothesis_encoded["input_ids"][0]).unsqueeze(0).to(device)

    # Create segment & masked position tensors (assuming no masking is needed)
    segment_ids = torch.zeros_like(premise_tokens).to(device)
    masked_positions = torch.zeros_like(premise_tokens).to(device)

    # Extract sentence embeddings
    with torch.no_grad():
        premise_embedding, _, _ = model(premise_tokens, segment_ids, masked_positions)
        hypothesis_embedding, _, _ = model(hypothesis_tokens, segment_ids, masked_positions)

    # Get CLS token representation
    cls_representation = premise_embedding[:, 0, :]  # Shape: (1, hidden_dim)

    # Pass through classifier head
    with torch.no_grad():
        logits = model.classifier(cls_representation)
